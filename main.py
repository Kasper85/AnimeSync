...
async def run_scraper():
    ...
    async with async_playwright() as p:
        ...
        for idx, url_base in enumerate(urls, 1):
            ...
            print("\n" + "="*50)
            print(f"📊 ESTADÍSTICAS SERIE: {nombre_serie}")
            print("="*50)
            print(f"⏱️ Tiempo: {tiempo_serie:.2f} seg")
            print(f"💾 Descargado: {megas_serie:.2f} MB")
            print("="*50 + "\n")
    ...
    if len(urls) > 1:
        print("\n" + "="*60)
        print("📊 ESTADÍSTICAS GLOBALES (TODAS LAS SERIES)")
        print("="*60)
        print(f"⏱️ Tiempo total: {tiempo_total_app:.2f} seg")
        print(f"💾 Descargado total: {megas_totales:.2f} MB")
        print("="*60 + "\n")
